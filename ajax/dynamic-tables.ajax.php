	...
	public function loadAjaxTable(){
		...
		if($this->search != ""){
			...
			foreach ($linkTo as $key => $value) {
				...
				if($table->status == 200){
					...
					if($this->idOffice > 0 && $module->title_module == "offices"){
						...
						$url = $module->title_module."?linkTo=".$value.",id_office&search=".str_replace(" ", "_", $this->search).",".$this->idOffice."&select=id_".$module->suffix_module;
			...
			if($this->idOffice > 0 && $module->title_module == "offices"){
				...
				$url = $module->title_module."?linkTo=date_created_".$module->suffix_module."&between1=".$this->between1."&between2=".$this->between2."&orderBy=".$this->orderBy."&orderMode=".$this->orderMode."&startAt=".$startAt."&endAt=".$this->limit."&filterTo=id_office&inTo=".$this->idOffice;	
...
if(isset($_POST["contentModule"])){
    ...
    $ajax -> idOffice = base64_decode($_POST["idOffice"]);    
    $ajax -> loadAjaxTable();
