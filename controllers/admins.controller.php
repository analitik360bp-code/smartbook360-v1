	...
	public function login(){
		...
		if(isset($_POST["email_admin"])){
			...
			if($login->status == 200){
				...
				// Establecer la zona horaria (opcional, pero muy recomendado)
				...
				=============================================*/
				$url_suscripcion = "suscripciones?linkTo=id_office_suscripcion&equalTo=".$login->results[0]->id_office_admin;
				$method_suscripcion = "GET";
				$fields_suscripcion = array();

				$suscripcion = CurlController::request($url_suscripcion,$method_suscripcion,$fields_suscripcion);
				
				if($login->results[0]->status_admin == 0 || $suscripcion->results[0]->end_date_suscripcion <= $fecha_hoy ){
					if($login->results[0]->rol_admin != "superadmin" && $suscripcion->results[0]->end_date_suscripcion <= $fecha_hoy){
						echo '<div class="alert alert-danger mt-3 rounded">Error al ingresar: Cuenta inactiva, renueva tu membresía</div>

						<script>

							fncMatPreloader("off");
							fncFormatInputs();
							fncToastr("error", "Error al ingresar: Cuenta inactiva, renueva tu membresía");
