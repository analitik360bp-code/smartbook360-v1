...
if($module->title_module == "offices" && $_SESSION["admin"]->id_office_admin > 0){
	...
	$url = $module->title_module."?linkTo=id_".$module->suffix_module."&equalTo=".$_SESSION["admin"]->id_office_admin."&orderBy=id_".$module->suffix_module."&orderMode=DESC&startAt=0&endAt=".$limit;
	        					...
	        					<?php foreach ($columns as $index => $item): ?>
	        						...
	        						<?php if ($item->visible_column == 1): ?>
										...
										=============================================*/
											...
											if($item->title_column == "transaction_order"){

												echo '<a href="/pos?transactionOrder='.urldecode($value[$item->title_column]).'" class="btn btn-dark btn-sm rounded">'.TemplateController::reduceText(urldecode($value[$item->title_column]),25).'<a>';
											}else{
